...
def V1XXX_home_page():
    ...
    st.markdown('<div class="title">V1XXX 장비</div>', unsafe_allow_html=True)

    # V1XXX 장비 이미지 추가
    image_path = "C:\\Users\\LG\\Desktop\\handbook\\장비사진\\V1XXX.png"  # 이미지 경로를 수정
    try:
...
def V1XXX_e_page():
   ...
   # 🔷 포트 설정 구간
    ...
    data = [
        ["V1XXX# conf t", "컨피그 모드 진입"],
        ["V1XXX(config)# int br1", "인터페이스 진입"],
        ["V1XXX(config-if)# no shutdown", "인터페이스 활성화"],
        ["V1XXX(config-if)# ip addr 10.0.0.2/24", "장비 IP 설정"],
        ["V1XXX(config)# ip route 0.0.0.0/0 10.0.0.1", "기본 게이트웨이 설정"],
        ["V1XXX# show ip", "IP 설정 확인"]
    ]

    data1 = [
        ["SWITCH# conf t", "컨피그 모드 진입"],
        ["SWITCH(config)# hostname V1XXX", "호스트 이름 변경"],
        ["V1XXX(config)# passwd", "패스워드 설정"]
    ]

    data2 = [
        ["V1XXX# conf t", "컨피그 모드 진입"],
        ["V1XXX(config)# br", "브리지 모드 전환"],
        [f"V1XXX(bridge)# filter dhcp add {port_min}~{port_max}", f"DHCP 필터 적용 ({port_min}~{port_max}포트)"],
        [f"V1XXX(bridge)# filter netbios add {port_min}~{port_max}", f"NetBIOS 필터 적용 ({port_min}~{port_max}포트)"]
    ]

    data3 = [
        ["V1XXX# conf t", "컨피그 모드 진입"],
        ["V1XXX(config)# br", "브리지 모드 전환"],
        [f"V1XXX(bridge)# set lock-port {port_min}~{port_max} enable", "포트 고정 설정"],
        [f"V1XXX(bridge)# set max-host {port_min}~{port_max} 1", "포트당 MAC 1개 제한"],
        [f"V1XXX(bridge)# set storm-control {port_min}~{port_max} 400", "Broadcast 400pps 제한"]
    ]
    ...
    st.markdown('<div class="title">MAX-host 및 Storm-control 설정</div>', unsafe_allow_html=True)
...
def V1XXX_w_page():
    ...
    commands = [
        ...
        ["show port 1-26", "시스템이 각 모듈을 인식하는지 점검"],
        ...
        ["show filter", "필터링(Netbios,Dhcp) 설정 내용 확인"],
        ["show max-hosts", "포트별 Mac수 제한 설정값 확인"],
        
        ["conf t\nbridge\nset port disable 1", "특정 포트 리셋 (비활성화)"],
        ["conf t\nbridge\nset port enable 1", "특정 포트 리셋 (활성화)"],
        ["conf t\nbridge\nset max-hosts 1 3", "1번 포트 → Mac 제한 3개로 설정"],
        ["conf t\nbridge\nclear max-hosts 24", "uplink 포트 Mac제한 무제한 설정"],
    
        ["show mac br 1", "특정 포트의 MAC address 확인"],
        ["show mac br1", "포트별 MAC address 확인, 링크 상태 확인"],
        ["ping 168.126.63.1", "PING TEST 수행"],
        ["sping 168.126.63.1 218.153.41.229", "SOURCE PING 수행 (source ip destination ip)"],
        ["show interface br1(interface명)", "각 포트별 collision과 packet 상태 확인"],
        ["show port statistics avg-pkt 1", "포트 트래픽 통계 (평균 패킷)"],
        ["show port statistics mmon 1", "지정 포트의 MIB, CRC 정보 확인"],
        ["show igmp", "멀티캐스트 서비스 확인"],
        ["conf t\nclear port statistics 1", "포트 통계 초기화"],
        ["conf t\nrestore factory-defaults", "시스템 초기화 (공장초기화)"],
        ["reload", "시스템 리부팅"],
        ["conf t\npasswd", "패스워드 변경"]
    ]
