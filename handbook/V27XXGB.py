...
def V27XXGB_home_page():
    ...
    st.markdown('<div class="title">V27XXGB 장비</div>', unsafe_allow_html=True)

    # V27XXGB 장비 이미지 추가
    image_path = "C:\\Users\\LG\\Desktop\\handbook\\장비사진\\V27XXGB.png"  # 이미지 경로를 수정
    try:
...
def V27XXGB_e_page():
   ...
   # 🔷 포트 설정 구간
    ...
    data = [
        ["V27XXGB# conf t", "컨피그 모드 진입"],
        ["V27XXGB(config)# int vlan1", "업링크 인터페이스 진입"],
        ["V27XXGB(config-if-vlan1)# no shutdown", "인터페이스 활성화"],
        ["V27XXGB(config-if-vlan1)# ip addr 10.0.0.2/24", "장비 IP설정"],
        ["V27XXGB(config-if-vlan1)# ip default-gateway 10.0.0.1", "DEFAULT G/W설정"],
        ["V27XXGB# show ip int brief", "장비 IP설정 확인"]
    ]
    data1 = [
        ["SWITCH# conf t", "컨피그 모드 진입"],
        ["SWITCH(config)# hostname {equipment}", "장비명 변경"],
        ["V27XXGB(config)# username root password test", "패스워드 설정"],
        ["V27XXGB(config)# enable password test", "enable 패스워드 설정"],
        ["V27XXGB(config)# no enable password", "enable 패스워드 삭제"]
    ]
    data2 = [
        ["SWITCH# conf t", "컨피그 모드 진입"],
        ["V27XXGB(config)# range port", "인터페이스 범위 설정모드 진입"],
        ...
        [f"E56XXR(config-range-port)# no mac-count {gi_cmd}", f"{gi_cmd} 포트 MAC 제한 해제"],
        ["V27XXGB# sh mac-count", "MAC 허용 개수 및 사용 개수 확인"]
    ]
...
def V27XXGB_w_page():
    ...
    commands = [
        ...
        ["show port statistics avg", "모든 포트의 평균 트래픽 통계 확인"],
        ["show port statistics avg 1", "포트 1의 평균 트래픽 통계 확인"],
        ["show port statistics avg type", "포트 유형별 평균 트래픽 통계 확인"],
        ["show port statistics avg type 1", "포트 1의 특정 유형 평균 트래픽 통계 확인"],
        ["show cable-length 1-{port_count}", "1~{port_count}번 포트의 케이블 길이 측정"],
        ["show cable-length disconnected 1-{port_count}", "비연결 포트의 케이블 길이 측정"],
        ["show arp", "ARP 테이블 확인"],
        ["show mac", "MAC 주소 테이블 확인"],
        ["show ip igmp snooping groups", "IGMP 그룹 정보 확인"],
        ...
        ["show cpe mac 1-24", "1~24번 포트의 CPE MAC 주소 정보 확인"],
...
def main():
    st.sidebar.title("V27XXGB 장비 핸드북")
