...
def E50XXH_home_page():
    ...
    st.markdown('<div class="title">E50XXH 장비</div>', unsafe_allow_html=True)

    # E50XXH 장비 이미지 추가
    image_path = "C:\\Users\\LG\\Desktop\\handbook\\장비사진\\E50XXH.png"  # 이미지 경로를 수정
    try:
...
def E50XXH_e_page():
   ...
   # 🔷 포트 설정 구간
    ...
    # 🔸 장비 IP 설정 명령어
    data = [
        ["E50XX# conf t", "컨피그 모드 진입"],
        ["E50XXH(config)# int vlan1", "업링크 인터페이스 진입"],
        ["E50XXH(config-if-vlan1)# no shutdown", "인터페이스 활성화"],
        ["E50XXH(config-if-vlan1)# ip addr 10.0.0.2/24", "장비 IP설정"],
        ["E50XXH(config-if-vlan1)# ip default-gateway 10.0.0.1", "DEFAULT G/W설정"],
        ["E50XXH# show ip int brief", "장비 IP설정 확인"]
    ]

    # 🔸 장비 이름 및 비밀번호 설정 명령어
    data1 = [
        ["SWITCH# conf t", "컨피그 모드 진입"],
        ["SWITCH(config)# hostname E50XXH", "장비명 변경"],
        ["E50XXH(config)# username root password test", "패스워드 설정"],
        ["E50XXH(config)# enable password test", "enable 패스워드 설정"],
        ["E50XXH(config)# no enable password", "enable 패스워드 삭제"]
    ]

    # 🔸 포트 설정 명령어 (입력 값 반영)
    data2 = [
        ["SWITCH# conf t", "컨피그 모드 진입"],
        ["E50XXH(config)# range port", "인터페이스 범위 설정모드 진입"],
        [f"E50XXH(config-range-port)# no shutdown {port_min}~{port_max}", f"{port_min}~{port_max}번 포트 개방"],
        [f"E50XXH(config-range-port)# mac-count 8 {port_min}~{port_max}", f"{port_min}~{port_max}번 포트 MAC 제한"],
        [f"E50XXH(config-range-port)# no mac-count {port_max}", f"{port_max} 포트 MAC 제한 해제"],
        ["E50XXH# sh mac-count", "MAC 허용 개수 및 사용 개수 확인"]
    ]
...
def E50XXH_w_page():
    ...
    commands = [
        ...
        ["show port status", "포트상태 확인"],
        ["show ip dhcp snooping binding", "IP할당 상태 확인"],
        ["show port counter", "포트 트래픽 통계"],
        ["show port statistics all", "포트의 평균 트래픽을 출력"],
        ["show port cpe lan-port status gi1", "CPE 가입자 포트상태 확인"],
        ["show port cpe count gi1", "CPE 가입자 포 통계치 확인"],
        ["show port phy-diag gi1", "GIGA가입자 cable 거리 확인"],
        ["clear counters", "포트통계 초기화"],
        ["show ip igmp snooping", "IGMP Snooping 기능 활성화 되어 있는지 확인"],
        ["show ip igmp snooping group", "가입자의 multicast 그룹 확인"],
        ["show ip igmp snooping mrouter", "multicast router 확인"],
        ["show ip igmp snooping querier", "snooping querier 포트 확인"],
        ["show ip igmp snooping mac-entry", "IPTV 서비스중인 가입자 포트 확인"],
        ["show ip igmp snooping reporter", "IGMP 통계 확인"],
        ["show ip igmp snooping statistics", "IGMP Proxy-reporting 상태 확인"],
        ["erase startup-config", "공장초기값 설정"],
        ["reload", "시스템 리부팅"]
    ]

    # HTML 테이블 생성
    st.markdown('<div class="title">네트워크 장비 조회 명령어</div>', unsafe_allow_html=True)
    components.html(generate_table_html_w(commands), height=1000)  # height를 설정하지 않고 테이블 크기를 자동으로 맞추도록 설정
...
def main():
    st.sidebar.title("E50XXH 장비 핸드북")
